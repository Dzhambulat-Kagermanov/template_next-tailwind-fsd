...
const triggerAreaCls =
  'flex items-center justify-between py-2 px-3 gap-x-2 cursor-pointer'
const iconCls = 'duration-150'
const iconExpandedCls = '-rotate-[180deg]'

interface Props
  extends TClassName,
    Omit<TTriggerAreaData, 'defaultActiveItemValue'> {
...
const TriggerArea: FC<Props> = ({
  ...
  return (
    <div className={cn([triggerAreaCls, className])} onClick={handleExpand}>
      ...
      {icon || (
        <MarkArrowIcon
          className={cn([iconCls], { [iconExpandedCls]: isExpanded })}
        />
      )}
    </div>
  )
